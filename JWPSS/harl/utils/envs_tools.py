...
def get_shape_from_obs_space(obs_space):
    ...
    """
    # print("obs_space.__class__.__name__",obs_space.__class__.__name__)
...
def make_train_env(env_name, seed, n_threads, env_args,index):
    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
                ...
                from harl.envs.WS_simulator.NormalWPSEnv import WPSEnv
                env = WPSEnv(env_args,index)
            else:
...
def make_eval_env(env_name, seed, n_threads, env_args,index):
