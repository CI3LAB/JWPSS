...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if self.algo_args["render"]["use_render"]:  # make envs for rendering
            ...
            self.Env_train(args,algo_args,env_args,0)
            self.Env_eval(args,algo_args,env_args,0)
    ...
    def Env_train(self,args,algo_args,env_args,index):
    ...
    def Env_eval(self,args,algo_args,env_args,index):
        self.eval_envs = (
            make_eval_env(
                args["env"],
                algo_args["seed"]["seed"],
    ...
    def run(self):
        ...
        for episode in range(1, episodes + 1):
            if episode != 0 and episode // 40 != (episode - 1) // 40:
                self.envs.close()
                self.Env_train(self.args, self.algo_args, self.env_args, episode // 40)
                self.warmup_init()
                print("UPDATA TRAIN_ENVS********************************")
            ...
            for step in range(self.algo_args["train"]["episode_length"]):
                ...
                (
                    ...
                    dones,
                    infos,
                    available_actions,obj,individual
                ...
                self.logger.per_step(data,obj)  # logger callback at each step
            ...
            if episode % self.algo_args["train"]["eval_interval"] == 0:
                if episode != 0 and episode // 100 != (episode - 1) // 100:
                    self.eval_envs.close()
                    self.Env_eval(self.args, self.algo_args, self.env_args, 250+episode // 100)
                    print("UPDATA EVAL_ENVS********************************")
    ...
    def collect(self, step):
        ...
        for agent_id in range(self.num_agents):
            available_action_agent = self.envs.get_available_actions(agent_id)
            action, action_log_prob, rnn_state = self.actor[agent_id].get_actions(
                ...
                available_action_agent.T,
                self.actor_buffer[agent_id].available_actions[step]
                if self.actor_buffer[agent_id].available_actions is not None
                else None,
            )
            self.envs.step_agent(agent_id,action)
    ...
    def eval(self):
        ...
        while True:
            # print("start eval********************************",eval_episode)
            eval_actions_collector = []
            for agent_id in range(self.num_agents):
                eval_available_actions_agent = self.eval_envs.get_available_actions(agent_id)
                # print("eval_available_actions_agent: ", eval_available_actions_agent)
                eval_actions, temp_rnn_state = self.actor[agent_id].act(
                    eval_obs[:, agent_id],
                    eval_rnn_states[:, agent_id],
                    eval_masks[:, agent_id],
                    eval_available_actions_agent.T,
                    eval_available_actions[:, agent_id]
                    if eval_available_actions[0] is not None
                    else None,
                    deterministic=True,
                )
                self.eval_envs.step_agent(agent_id, eval_actions)
