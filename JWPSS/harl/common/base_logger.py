...
class BaseLogger:
    ...
    def episode_log(
        ...
        if len(self.done_episodes_rewards) > 0:
            ...
            aver_episode_objective = np.mean(self.obj)
            # print("Some episodes done, average episode reward is {}.".format(aver_episode_rewards))
            print(
            ...
            print(
                "Some episodes done, average episode objective is {}.\n".format(
                    aver_episode_objective
                )
            )
            ...
            self.writter.add_scalars(
                "train_episode_objective",
                {"average_objective": aver_episode_objective},
                self.total_num_steps,
            )
            self.done_episodes_rewards = []
            self.obj = []
    ...
    def eval_thread_done(self, tid, obj, individual):
        """Log evaluation information."""
        self.eval_episode_rewards[tid].append(
            np.sum(self.one_episode_rewards[tid], axis=0)
        )
        self.eval_obj.append(obj)
        self.eval_individual.append(individual)
    ...
    def eval_log(self, eval_episode):
        ...
        eval_env_infos = {
            ...
            "eval_average_episode_objective": self.eval_obj,
        }
        ...
        self.log_file.flush()
        self.log_file.write(
            ",".join(map(str, [self.total_num_steps, self.eval_individual])) + "\n"
        )
        self.log_file.flush()
